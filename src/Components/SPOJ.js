...
const SPOJ = () => {
  ...
  useEffect(() => {
    ...
    const fetchPlatformData = async () => {
      try 
      {
        const uname = "sasireka_13"
        const data = {}
        const response = await axios.get(`http://localhost:5000/spoj/${uname}`);
  ...
  return (
    <div className="SPOJ">
        <Platform logo={Logo[5]} name="SPOJ" userName={platformData.userName} features={platformData.features}></Platform>
    </div>
  );
};
